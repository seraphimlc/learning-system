...
## 题目质量三闸门（2026-08-20 新增）

生成管线三道质量门禁（契约 v1.1 + 琢玉审查实测验证）：

1. **sympy 正确性门禁**：答案机器验算，mismatch 不入库（已有）
2. **契约校验**：≥2 题型族指纹 / ≥1 transfer / 难度分布 / design_rationale 五要素齐全 / 琐碎题启发式警告
3. **教研审查门禁**（琢玉，注入式 reviewer_fn）：超纲 / 偏题 / 表述 / 难度诚实（反注水）/ 模板反射 / 错因陷阱——试点 30 题实测抓到 3 偏题 + 6 难度虚高 + 2 答案歧义（机器门禁抓不到），已修订重跑通过
