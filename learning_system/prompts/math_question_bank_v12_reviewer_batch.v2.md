# Math Question Bank v12 Reviewer Batch Prompt v2
...
Score each dimension from 0.0 to 1.0. An active item requires every general score at least 0.80, every v2 child-surface semantic score and assessment at least 0.88, reviewer confidence at least 0.88, all evidence flags true, and verdict `approved`.
...
- Record `semantic_evidence.child_surface_readiness` and `scores.child_surface_readiness`. A failed verdict or score below 0.88 forbids approval.

### Single Coherent Response Burden
...
- Record `semantic_evidence.single_coherent_response_burden` and `scores.response_burden_fit`. A failed verdict or score below 0.88 forbids approval.

### Unprompted Process Integrity

- When the runtime marks the candidate `unprompted_process_evidence`, set applicability to `required`; approval requires verdict `satisfied` and score at least 0.88.
...
4. Score all general and v2 child-surface dimensions without trusting candidate metadata.
...
8. Fill every `semantic_evidence` assessment. The verdict/score/reason must reflect the actual prompt; metadata declarations are not evidence.
...
Return only JSON matching `2026-07-12.math-qb-v12.reviewer.schema.v2`. The `item_reviews` array must review only the submitted candidates for `trusted_context.requested_slots`; do not create reviews for absent slots. Every review must include v2 semantic evidence. Do not include markdown, hidden chain-of-thought, or fields outside the schema. Reasons should be concise review evidence, not private reasoning traces.
