...
PILOT_VERSION = "2026-07-26.math-three-node-pilot.v1"
PILOT_MANIFEST_ID = "math_three_node_pilot_2026_07_26_v1"
PILOT_NODE_IDS = (
    "M-PRE-INTEGER-OPS",
    "M-G7-NUMBER-LINE",
    "M-G7-EQ-SOLVE",
...
PILOT_ITEM_COUNT = 18
PILOT_ACTIVE_ITEM_COUNT = 17
ITEMS_PER_NODE = 6
DEFAULT_MANIFEST = (
    PROJECT_ROOT
    / "data/question_banks/math/math_three_node_pilot_2026-07-26.v1.json"
)
...
def _dimension_for_target(key: str) -> str:
    normalized = key.lower()
    if any(token in normalized for token in ("check", "verify", "range")):
        return "check"
    if any(token in normalized for token in ("error", "reason", "meaning")):
        return "concept"
    if any(token in normalized for token in ("number_line", "point", "position", "representation")):
        return "representation"
    if any(token in normalized for token in ("relation", "equivalence", "direction", "distance", "sign")):
        return "model_relation"
    if any(token in normalized for token in ("process", "work", "operation", "intermediate", "regroup", "division", "corrected")):
        return "procedure"
    if any(token in normalized for token in ("result", "answer", "solution", "coordinate", "order", "value")):
        return "final_answer"
    return "concept"
        ...
        targets.append(
            {
                ...
                "criterion": criterion,
                "points": points,
                "dimension": _dimension_for_target(key),
...
def _build_item(
    ...
    *,
    ...
    if usage not in {"diagnostic", "practice"}:
    ...
    family_id = "PF-PILOT-" + canonical_sha256(
        {"manifest_id": manifest["manifest_id"], "node_id": node_id, "role": role}
    )[:16]
    ...
    instance_basis = {"instance_key": instance_key, "problem": prompt}
...
def load_pilot(
    ...
    *,
    ...
    if not isinstance(raw_items, list) or len(raw_items) != PILOT_ITEM_COUNT:
        raise ValueError("pilot manifest must contain exactly 18 items")
    ...
    if node_counts != Counter({node_id: ITEMS_PER_NODE for node_id in PILOT_NODE_IDS}):
        raise ValueError("pilot manifest must contain exactly six items for each node")
    ...
    if any(role_counts[(node_id, role)] != 1 for node_id in PILOT_NODE_IDS for role in ROLE_TO_KIND):
        raise ValueError("pilot manifest must contain each pedagogical role once per node")
...
def _verify_installation(
    ...
    *,
    ...
    if len(rows) != PILOT_ACTIVE_ITEM_COUNT:
        raise ValueError("pilot database does not contain exactly 17 active questions")
    expected_by_id = {
        item["id"]: item
        for item in items
...
def activate(
    ...
    *,
    ...
    if resolved_db_path == PRODUCTION_DB:
    ...
    try:
        ...
        if existing_rows or existing_ledgers:
        ...
        with conn:
            ...
            for item in items:
                ...
                reviewer_run = db.record_agent_run(
                    ...
                    output={
                        ...
                        "active_eligible": item.get("activation_status") == "active",
            ...
            ledger = db.activate_question_bank_version(
                ...
                reason="isolated activation of expert-reviewed three-node pilot",
                commit=False,
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = activate(args.db, manifest_path=args.manifest)
